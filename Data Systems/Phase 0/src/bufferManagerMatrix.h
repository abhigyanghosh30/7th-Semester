...
/**
 * @brief The BufferManagerMatrix is responsible for reading pages to the main memory.
 ...
 *
 */
class BufferManagerMatrix
{

    deque<PageMatrix> pages;
    bool inPool(string pageName);
    PageMatrix getFromPool(string pageName);
    PageMatrix insertIntoPool(string tableName, int pageIndex);

public:
    BufferManagerMatrix();
    PageMatrix getPage(string tableName, int pageIndex);
    void writePage(string pageName, vector<int> values);
    void deleteFile(string tableName, int pageIndex);
    void deleteFile(string fileName);
    void writePage(string tableName, int pageIndex, vector<int> values, uint pageSize);
};